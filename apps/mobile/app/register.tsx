      ...
      try {
        ...
        if (res.ok) {
          ...
          if (
            ...
            const reset = () =>
              setForm((prev) => ({ ...prev, instance: cloudInstance }));
    ...
    <>
      <KeyboardStickyView className="flex-col justify-end h-full bg-base-100 relative">
        <View className="flex-col justify-end h-full bg-primary relative">
          ...
          <Text className="text-base-100 text-5xl font-bold ml-8">
            {sentTo ? "Check Email" : "Sign Up"}
          </Text>
          <View>
            <Text
              ...
              numberOfLines={1}
            >
              {sentTo ? sentTo.email : `Sign up to ${instanceName}`}
            </Text>
            {!sentTo && (
              <TouchableOpacity
                onPress={() => {
                  if (showInstanceField) {
                    setForm({ ...form, instance: cloudInstance });
                  }
          ...
          <SafeAreaView
            edges={["bottom"]}
            className="flex-col justify-end h-auto duration-100 pt-8 bg-base-100 -mt-2 pb-10 gap-3 w-full px-4"
          >
            {sentTo ? (
              <>
                <Text className="text-base-content text-center text-base px-2">
                  We sent you a verification link. After verifying your email,
              ...
              <>
                ...
                {isConfigLoading && instance !== cloudInstance && (
                  <View className="flex-row justify-center items-center gap-2">
                    <ActivityIndicator />
                    <Text className="text-neutral">Checking server...</Text>
                  </View>
                )}
                ...
                {currentConfig?.EMAIL_PROVIDER === true &&
                  !supportsMobileSignup && (
                    <Text className="text-neutral text-center">
                      This instance needs to be updated to support mobile
                      sign-up. You can sign up through the web, then come back
                      here to log in.
                    </Text>
                  )}
                <Input
